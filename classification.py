...
# 2. Clés AWS et connexion à S3 + Bedrock
...
# 3. Télécharger le CSV depuis S3
...
print("CSV chargé. Extrait des 3 premières lignes :")
print(df.head(3))
...
# 5. Lister les 500 images du dossier rakutenpremieresimagesdudossier/ (tri décroissant)
...
bucket_name = "rakutenpremieresimagesdudossier"  # ✅ c'est le nom du BUCKET

image_keys = list_sorted_image_keys(bucket_name, prefix="", limit=500)

# 🧩 Construire image_map avec seulement les 500 images
...
print(f"✅ {len(image_map)} images dans le mapping")
print("🔍 Exemple image_map :")
print(list(image_map.items())[:3])

# 6. Prendre les 500 dernières lignes du CSV (ordre décroissant)
...
print(f"✅ {len(df_500)} lignes sélectionnées avec image dispo")

# 4. Liste des catégories Rakuten
categories = [
    "Livre", "Musique CD", "DVD Blu-Ray", "Jeux vidéo Console", "Téléphonie Tablette",
    "Informatique Logiciel", "TV Image et Son", "Maison", "Electroménager", "Alimentation Boisson",
    "Brico Jardin Animalerie", "Sport Loisirs", "Mode", "Beauté", "Jouet Enfant Puériculture",
...
# 5. Indexer toutes les images valides (présentes dans le CSV)
pattern = re.compile(r'image_(\d+)_product')  # extrait image_id à partir du nom du fichier
csv_imageids = set(df['imageid'].astype(str))  # liste des image_id présents dans le CSV
...
# Dictionnaire image_id → chemin S3 de l’image
image_map = {}

# Explorer les deux dossiers d’images
...
print(f"Nombre d’images filtrées liées au CSV : {len(image_map)}")
print("Exemples de correspondances image_id → image_key :")
for i, (img_id, s3_key) in enumerate(image_map.items()):
    print(f"{img_id} → {s3_key}")
...
def get_image_base64_from_s3(image_key):
    try:
        ...
        print(f"Erreur téléchargement image {image_key} : {e}")
        return None
...
def call_bedrock_with_image_and_text(description, designation, image_b64):
    ...
    prompt = (
        "Tu es un agent IA de catégorisation pour Rakuten. "
        "Voici un produit à classer dans l'une des catégories suivantes :\n"
        f"{categories_str}\n\n"
        f"Données produit :\n"
        f"- Nom ou désignation : {champ_utilise}\n"
        f"- Image encodée en base64 (indice visuel) : {image_b64[:300]}...\n\n"
        "Réponds uniquement par le nom de la catégorie la plus probable, sans explication."
    )
    ...
    try:
        ...
        if "content" not in result or not result["content"]:
            print(f"Réponse vide ou inattendue : {result}")
            return "Erreur"
...
for idx, row in tqdm(df_500.iterrows(), total=len(df_500)):
    try:
        ...
        cat = "Erreur"
    results.append(cat)
    time.sleep(0.2)  # limite de requêtes Bedrock
...
print("Image Key trouvé dans image_map :", image_key)

try:
    obj = s3.get_object(Bucket=bucket_name, Key=image_key)
    print("✔️ Image trouvée dans S3. Taille :", obj['ContentLength'], "bytes")
except Exception as e:
    print("Erreur accès S3 :", e)
...
# Cherche s'il y a des IDs du CSV qui apparaissent en partie dans les clés S3 (diagnostic manuel)
print("🔍 CSV imageids présents dans image_map ?")
for i in range(5):
    imageid = csv_imageids[i]
    s3_keys = [v for k, v in image_map.items() if imageid in v]
    print(f"imageid {imageid} → trouvé dans S3 ? {'✅' if s3_keys else '❌'}")
    if s3_keys:
        print(f"  → Exemple clé : {s3_keys[0]}")
...
prefix = "images/"      # ex: "images/" ou "" si tes images sont à la racine

aws_access_key_id = "TA_CLE_ACCESS_KEY_ID"
aws_secret_access_key = "TON_SECRET_ACCESS_KEY"
region_name = "us-west-2"  # adapte à ta région S3
...
print(f"{len(image_ids_available)} images trouvées sur S3")
